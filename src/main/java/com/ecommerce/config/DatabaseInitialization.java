...
public class DatabaseInitialization implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // 1. Fix Product Status: Set to 'ACTIVE' if status is NULL or not 'ACTIVE' for
        // existing products
        try {
            ...
            if (activeCount == 0) {
                int updated = jdbcTemplate
                        .update("UPDATE products SET status = 'ACTIVE' WHERE status IS NULL OR status != 'ACTIVE'");
        ...
        // 2. Ensure Slug uniqueness if needed (but Hibernate should handle that via
        // schema)

        // 3. Fix Shop status if needed
        try {
            int updatedShops = jdbcTemplate
                    .update("UPDATE shops SET status = 'ACTIVE' WHERE status IS NULL OR status = ''");
            if (updatedShops > 0) {
